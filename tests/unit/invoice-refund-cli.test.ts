...
describe("invoice refund CLI", () => {
  test("refunds a credited invoice via invoice-number input json", async () => {
    const root = mkdtempSync(join(tmpdir(), "rentemester-invoice-refund-cli-"));
    const company = join(root, "company");
    const refundJson = join(root, "refund.json");
    ...
    await Bun.$`bun run src/cli.ts invoice post --company ${company} --document-id ${issued.documentId}`.quiet();
    ...
    writeFileSync(refundJson, JSON.stringify({
      invoiceNumber: issued.invoiceNumber,
      bankTransactionReference: "RFND-0001"
    }, null, 2));

    const proc = Bun.spawn(["bun", "run", "src/cli.ts", "invoice", "refund-bank", "--company", company, "--input", refundJson], {
      cwd: process.cwd(),
      stdout: "pipe",
      stderr: "pipe",
    });
